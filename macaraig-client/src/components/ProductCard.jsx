import Button from './Button';



const ProductCard = ({ product, index }) => {
  return (
    <article className="rounded-3xl border-2 border-zinc-900 bg-blue-950 p-4">
      <div className="flex aspect-4/3 items-center justify-center rounded-[1.25rem] bg-zinc-200">
        <img src={product.image} alt={product.title} className="rounded-[1.25rem]" />
      </div>
      <p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.24em] text-white">
        {product.category} {String(index + 1).padStart(2, '0')}
      </p>
      <h3 className="mt-2 text-lg font-semibold text-white">{product.title}</h3>
      <p className="mt-2 text-base font-bold text-white">{product.price}</p>
      <p className="mt-3 text-sm leading-6 text-white">
        {product.content[0].substring(0, 120)}...
      </p>
      <div className="mt-4 flex justify-center">
        <Button to={`/products/${product.name}`} className="mt-4" variant="primary">
          View Product
        </Button>
      </div>
    </article>
  );
};

export default ProductCard;
